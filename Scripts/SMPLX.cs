...
public class SMPLX : MonoBehaviour
{
    ...
    {
        fileimport = GetComponent<FileImport>();
        if (!HasBetaShapes())
        {
        ...
        {
            print(fileimport.ReadBetaFile()[i]);
            _smr.SetBlendShapeWeight(i, float.Parse(fileimport.ReadBetaFile()[i]) * 100); // blend shape weights are specified in percentage
    ...
    {
        speed = 15;
        body = GameObject.Find("smplx-female-rootOffset");
        int num = 0;
        motion = false;
        int numFile = fileimport.paths.Count;
        int NUM_ANGLES = (NUM_JOINTS) * 3;//Add global
        List<string> motionArr = fileimport.ReadTransformFile();
        double time = 1.0;
        List<string> poseArr = fileimport.ReadPoseFile();
        ResetBodyPose();
       // body.transform.position = new Vector3(transform.position.z, transform.position.y, transform.position.x);

        for (int i = 0, n = 0; i < NUM_ANGLES && n < fileimport.paths.Count; i += 3)////////////////////////////////////////////////////////////////////////////////////
        {
            float A = float.Parse(poseArr[i + (n * NUM_ANGLES )]);
            float B = float.Parse(poseArr[i + (n * NUM_ANGLES ) + 1]);
            float C = float.Parse(poseArr[i + (n * NUM_ANGLES ) + 2]);
            
            num++;
           // print("Point A: " + A + "Point B: " + B + "Point C: " + C + "File #: " + n + "Number: " + num + "I:  " + i);
            Quaternion quat = QuatFromRodrigues(A, B, C);
            if (Input.GetAxisRaw("Horizontal") > 0.5f || Input.GetAxisRaw("Horizontal") < -0.5f)
            {
              body.transform.Translate(new Vector3(0f, 0f, -Input.GetAxisRaw("Horizontal") * speed * Time.deltaTime));
            ...
            print("Point A: " + A + "Point B: " + B + "Point C: " + C + "File #: " + n);

            if (i >= NUM_ANGLES - 3 && n < fileimport.paths.Count)
                {

                 body.transform.position = new Vector3(0, 0, transform.position.x);
                if (Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.LeftArrow)|| Input.GetKey(KeyCode.Space))
                    {
                       
                    print(n);
                        float x = float.Parse(motionArr[n * 3 + 0]);
                        float y = float.Parse(motionArr[n * 3 + 1]);
                        float z = float.Parse(motionArr[n * 3 + 2]);
                        diff = -Input.GetAxisRaw("Horizontal") * speed * Time.deltaTime - x;
                        //print()
                      // body.transform.position = new Vector3(0, 0,diff );
                      body.transform.position = new Vector3(0, -y, -x);
                    // transform.position = new Vector3(-z, -y, transform.position.x);
                    // SetLocalJointRotation("pelvis", QuatFromRodrigues(0, 0, 0));
                    
                    n++;
                        i = 0;
                        while (time > 0)
                        {
                            time -= Time.deltaTime / .05;
                            yield return null;
                        }

                        SetLocalJointRotation(_bodyJointNames[i / 3], quat);
  ...
  /*  public void Transform() {
        ...
        body = GameObject.Find("smplx-female-rootOffset");
        int num = 0;
        int numFile = fileimport.movement.Count;
        double time = 1.0;
        List<string> motionArr = fileimport.ReadTransformFile();
        // ResetBodyPose();
      
      
            //for (int n = 0; n < fileimport.movement.Count; n += 3)////////////////////////////////////////////////////////////////////////////////////
            //{
                float A = float.Parse(motionArr[n + 0]);
                float B = float.Parse(motionArr[n + 1]);
                float C = float.Parse(motionArr[n + 2]);
                transform.position = new Vector3(A, B, C);
                body.transform.position = new Vector3(A, B, C);
    ...
    public void Motion(int a, int b, int c)
    {
  ...
  /*  public void InputKey()
    {
        fileimport = GetComponent<FileImport>();
        keyName = fileimport.key;

        Input.GetKey(fileimport.key);

        //Add key and name of animation
        //name correlation of file finds file and reads from specific file to
        //have a new animation 
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.LeftArrow)|| Input.GetKey(KeyCode.Space) && motion==true)
        {
            SetLocalJointRotation("pelvis", QuatFromRodrigues(0, 0, 0));
            StartCoroutine(Animation());
    ...
    /*void FixedUpdate()
    {
        ...
        if (motion)
        {
            if (n < fileimport.movement.Count)
                Transform();
            else
                motion= false;
            n += 3;
